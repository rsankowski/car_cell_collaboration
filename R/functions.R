...
hyper_test_n <- function(data = df, var1 = "Cluster", var2 = "Region") {
  ...
  for (i in unique(data[[var2]])) {
    ...
    p_hyper <- apply(test_df, MARGIN = 1, function(x) phyper(max(0,x[[1]]-1), x[[2]], x[[3]], x[[4]], lower.tail = F)) #probability to get q or more successes in populaton
    #p_hyper_upper <- apply(test_df, MARGIN = 1, function(x) phyper(max(0,x[[1]]-1), x[[2]], x[[3]], x[[4]], lower.tail = F)) #probability to get q or more successes in populaton
    p_hyper_log <- apply(test_df, MARGIN = 1, function(x) phyper(max(0,x[[1]]-1), x[[2]], x[[3]], x[[4]],log.p = T, lower.tail = F)) #probability to get q or more successes in populaton
    
    test_df$p_hyper <- p_hyper
    test_df$p_hyper_log <- p_hyper_log
    test_df$Cluster <- vars_df[[`var1`]]
    test_df$enrichment_var <- i
    .df <- .df %>%
      bind_rows(test_df[,c("q","m","n","cluster_size","p_hyper","p_hyper_log","Cluster","enrichment_var")])
  }
  
  
  .df$padj <- p.adjust(.df$p_hyper, method="fdr")
